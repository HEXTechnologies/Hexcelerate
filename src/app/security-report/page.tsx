...
export default function SecurityReport() {
  const [formData, setFormData] = useState({
    reporterName: {
      value: "",
      helper:
        "Your name will be kept confidential and only used to follow up if needed.",
    },
    reporterEmail: {
      value: "",
    ...
    issueType: {
      value: "",
      helper:
        "Common types include: Data Exposure, Authentication Issues, Access Control, etc.",
    },
    shortDescription: {
      value: "",
      helper:
        "Brief overview of the security issue - what is the main concern?",
    },
    howToReproduce: {
      value: "",
      helper:
        "Step-by-step instructions that would help our team recreate the issue.",
    },
    whereFound: {
      value: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({
        type: "success",
        message:
          "Report submitted successfully. Thank you for helping us improve security!",
      });
            ...
            <div className="card-body p-4">
              <div className={`text-center mb-4 ${styles.headerSection}`}>
                ...
                <p className={`text-muted ${styles.subtitle}`}>
                  Help us protect our users by reporting potential security
                  vulnerabilities. All reports will be investigated promptly.
                </p>
              </div>
                ...
                <div
                  className={`alert ${
                    status.type === "success"
                      ? styles.successAlert
                      : styles.errorAlert
                  } mt-4 d-flex align-items-center gap-2`}
                >
                  {status.type === "success" ? (
                    <CheckCircle size={18} />
                  ) : (
